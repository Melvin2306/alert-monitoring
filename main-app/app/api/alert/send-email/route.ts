...
import {
  createFinding,
  sendAlertEmail
} from "@/app/api/lib/email/sendEmail";
import {
  WatchMatch
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (testMode && testEmail && !testEmail.includes('@')) {
      return NextResponse.json(
        { message: "Invalid email format for test email" },
        { status: 400 }
      );
    }
    ...
    if (!checkResponse.ok) {
      console.log('📧 Alert check API error:', checkResponse.status, await checkResponse.text().catch(() => 'No response text'));
      return NextResponse.json(
    ...
    if (matches.length === 0) {
      ...
      return NextResponse.json(
        { message: "No matches found, no emails sent" },
        { status: 200 }
      );
    }
    ...
    const findings = matches.map(match => {
      ...
      const timestamp = match.lastChanged 
        ? new Date(match.lastChanged) 
        : new Date();
    ...
    const emailResults = await Promise.allSettled(emailPromises.map(p => p.catch(e => {
    ...
    if (failureCount > 0) {
      ...
      console.log('📧 Failed emails:', failedEmails.map(f => ({ 
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('📧 Parameters:', { 
      providedMatches: providedMatches ? `${providedMatches.length} matches provided` : 'No matches provided',
      checkRecent, 
      hours, 
      testMode, 
      testEmail,
