...
const createPool = () => {
  try {
    ...
    if (!connectionString) {
      console.log(
        "DATABASE_URL not found, attempting to build connection string from individual variables..."
      );
      ...
      console.log(
        `DB_USER: ${dbUser}, DB_PASSWORD: ${dbPassword ? "********" : undefined}, DB_NAME: ${dbName}`
      );

      if (dbUser && dbPassword && dbName) {
        connectionString = `postgresql://${dbUser}:${dbPassword}@${dbHost}:${dbPort}/${dbName}`;
        console.log(
          `Successfully built database connection string from individual environment variables. Host: ${dbHost}, Port: ${dbPort}, Database: ${dbName}`
        );
      } else {
        const missingVars = [];
        ...
        console.error(
          `Critical database configuration missing: ${missingVars.join(", ")} environment variables not set`
        );
    ...
    if (process.env.DB_USE_SSL === "true") {
      ...
      console.log(
        `SSL configuration enabled. rejectUnauthorized: ${process.env.DB_REJECT_UNAUTHORIZED === "true"}`
      );
    }
    ...
    pool
      ...
      .catch((err) => {
        ...
        console.error(
          ...
          process.env.DB_HOST || "(from connection string)",
          "Port:",
          process.env.POSTGRES_PORT || "5432",
          "Database:",
          process.env.DB_NAME || "(from connection string)"
        );
        ...
        console.error(
          "2. Verify that the database name exists and user has appropriate permissions"
        );
